...
export async function POST(req: Request) {
  ...
  // ─────────────────────────────────────────────────────────────────────
  // Complétion automatique de la poule (aller/retour des équipes connues)
  //
  // L'import DOFA ne ramène que les matchs de l'équipe suivie. Après
  // l'upsert, on génère les confrontations manquantes des AUTRES équipes
  // (double round-robin, matchs `source='manual'`, scores NULL) pour que
  // la liste des résultats reflète la poule complète. Idempotent — les
  // confrontations déjà en base ne sont jamais dupliquées ni réécrites.
  //
  // L'échec isolé de cette étape ne fait JAMAIS échouer l'import (règle
  // d'isolation des écritures) : il est loggé et compté dans `generationErrors`.
  let autoGenerated = 0;
  ...
  try {
    ...
    const knownTeams = (freshRows ?? []).flatMap((row) => {
      const entries = [
        row.home_cl_no != null && row.home_team_number != null
          ? { cl_no: row.home_cl_no, number: row.home_team_number, short_name: row.home_team ?? "" }
          : null,
        row.away_cl_no != null && row.away_team_number != null
          ? { cl_no: row.away_cl_no, number: row.away_team_number, short_name: row.away_team ?? "" }
          : null,
      ];
      return entries.filter((e): e is { cl_no: number; number: number; short_name: string } => e !== null);
    });

    const teams = collectPoolTeams(knownTeams);
    if (teams.length >= 2) {
      const fixtures = generatePoolFixtures(teams);
      const missing = missingPoolFixtureRows(fixtures, existingOrderedPairs(freshRows ?? []), championshipId);
      if (missing.length > 0) {
      ...
      autoGenerated = missing.length;
    }
  } catch (err) {
    console.error("[dofa/ingest] Erreur complétion automatique de la poule:", err);
    generationErrors = 1;
  }
  ...
  return NextResponse.json({
    ...
    source: "dofa_import",
    eventSync,
    poolGenerated: autoGenerated,
    poolGenerationErrors: generationErrors,
  });
}
